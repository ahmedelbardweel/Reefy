<x-app-layout>
    <x-slot name="header">
        <div class="d-flex align-items-center gap-3" style="direction: rtl;">
            <a href="{{ route('community.index') }}" class="btn btn-light rounded-circle shadow-sm border-0 d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
                <i class="bi bi-arrow-right fs-5"></i>
            </a>
            <div>
                <h2 class="h4 text-dark mb-0 font-weight-bold">عرض المنشور</h2>
            ...
            <div class="col-lg-7">
                <div class="card border-0 shadow-sm mb-4 p-0" style="border-radius: 15px; direction: rtl;">
                    ...
                    <div class="card-body px-4 pt-3">
                        <p class="text-dark mb-3" style="line-height: 1.6; font-size: 1.1rem;">{{ $post->content }}</p>
                        
                        @if($post->image_path)
                            <div class="rounded-3 overflow-hidden mb-3 border shadow-sm">
                                <img src="{{ asset('storage/' . $post->image_path) }}" class="w-100" style="max-height: 500px; object-fit: contain; background: #f8fafc;" alt="Post content">
                            </div>
                        @endif
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" 
                                ...
                                id="likeBtn{{ $post->id }}"
                                class="btn flex-grow-1 py-1 border-0 d-flex align-items-center justify-content-center gap-2 {{ $post->isLikedBy(auth()->user()) ? 'text-danger fw-bold bg-danger bg-opacity-10' : 'text-muted hover-bg-light' }}" style="border-radius: 10px; transition: all 0.2s;">
                                <i class="bi {{ $post->isLikedBy(auth()->user()) ? 'bi-heart-fill' : 'bi-heart' }}"></i>
                                إعجاب
                            </button>
                            
                            <button type="button" class="btn flex-grow-1 py-1 border-0 text-muted d-flex align-items-center justify-content-center gap-2 hover-bg-light" style="border-radius: 10px;" onclick="openComments({{ $post->id }})">
                                <i class="bi bi-chat-left-text"></i>
                                تعليق
                            </button>
                            <button type="button" class="btn flex-grow-1 py-1 border-0 text-muted d-flex align-items-center justify-content-center gap-2 hover-bg-light" style="border-radius: 10px;" onclick="shareLocalPost()">
    ...
    <style>
        ...
        .sheet-content { position: absolute; bottom: -100%; left: 0; width: 100%; height: 70%; background: white; border-radius: 20px 20px 0 0; transition: bottom 0.3s ease-out; display: flex; flex-direction: column; }
        .comment-bottom-sheet.show .sheet-content { bottom: 0; }
        .drag-handle { width: 40px; height: 5px; background: #e0e0e0; border-radius: 10px; margin: 10px auto; }
                ...
                html += `
                    <div class="d-flex gap-2 mb-3">
                        <div class="flex-shrink-0">
                            <div class="rounded-circle bg-light border d-flex align-items-center justify-content-center text-success fw-bold" style="width: 35px; height: 35px;">
                                ${comment.user ? comment.user.name.charAt(0) : 'ز'}
                            </div>
                        </div>
                        <div class="flex-grow-1">
                            <div class="bg-light p-2 rounded-3 px-3 shadow-sm">
                                <div class="fw-bold small">${comment.user ? comment.user.name : 'زائر ريفي'}</div>
                            ...
                            <div class="ms-4 mt-2 border-end pe-3" style="border-right: 2px solid #eee;">
            ...
            replies.forEach(reply => {
                html += `
                    <div class="d-flex gap-2 mb-2">
                        <div class="flex-shrink-0">
                            <div class="rounded-circle bg-white border d-flex align-items-center justify-content-center text-success fw-bold" style="width: 25px; height: 25px;">
                                ${reply.user ? reply.user.name.charAt(0) : 'ز'}
                            </div>
                        </div>
                        <div class="flex-grow-1">
                            <div class="bg-white border p-2 rounded-3 px-3">
                                <span class="fw-bold small">${reply.user ? reply.user.name : 'زائر ريفي'}</span>
                                <p class="mb-0 very-small mt-1">${reply.content}</p>
                            </div>
                        </div>
                    </div>
                `;
